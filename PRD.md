...
- Mobile apps or push notification channels (email only in v1; the notifier is pluggable for later).
...
### FR-2 Alerting
- FR-2.1 At most one digest email per poll cycle, sent only when there is something new to say.
...
- FR-2.4 Every email carries a verify-before-booking caveat and seats.aero attribution.
...
- FR-5.5 After ≥6 consecutive failed cycles, attempt a failure-notice email at most once per 24 h.

### FR-6 CLI
- FR-6.1 `serve` (service: web + scheduler), `search` (one-shot cycle, `--dry-run` prints the digest without sending or recording), `test-email`, `routes-audit` (per-route crawl-horizon report), `probe-british` (empirically test the undocumented BA source), `status`.
...
|---|---|---|---|
| **1 — Deal Engine MVP** | Headless core: API client, SQLite state, detection, dedupe, email digest, `search`/`test-email` CLI | FR-1, FR-2, FR-6 (partial) | A qualifying fixture deal produces exactly one email; a second identical run produces none; full test suite passes without an API key |
| **2 — Always-On Service** | Scheduler + minimal service API, aux CLI, launchd KeepAlive | FR-5, FR-6 | Two consecutive scheduled cycles recorded with no duplicate emails; `kill -9` auto-relaunches; concurrent run-now returns 409 |
...
- **Reliability**: all state in SQLite; crash or restart at any point loses no alert history and causes no duplicate emails (alert state is committed only after a successful send).
...
- **Honesty about staleness**: every surface (email, UI) shows data age and an estimated-price flag where applicable; cached data is never presented as bookable fact.
- **Attribution**: "Award availability data provided by seats.aero" on every email, UI screen, CLI output, and in the README (Pro API terms).
...
- **Gmail**: sending requires 2FA + an app password.
...
- A qualifying fixture deal produces exactly one email; repeat cycles stay silent until the deal improves ≥15% or reappears after ≥7 days.
...
- A new qualifying deal in the data source is visible in the Dashboard and emailed within one polling interval.
...
| SMTP failures | Retries; alert state only committed after send, so deals re-surface next cycle |
